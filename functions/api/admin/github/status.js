...
const worksJsonPath = 'src/data/works.json';
...
export async function onRequestGet({ request, env }) {
  ...
  try {
    const appJwt = await createGitHubAppJwt(config.value.appId, config.value.privateKey);
    const installationToken = await createInstallationToken(config.value.installationId, appJwt);
    const [repo, worksFile] = await Promise.all([
      githubFetch(`/repos/${config.value.owner}/${config.value.repo}`, installationToken),
      githubFetch(
        `/repos/${config.value.owner}/${config.value.repo}/contents/${encodeGitHubPath(worksJsonPath)}?ref=${encodeURIComponent(config.value.branch)}`,
        installationToken
      )
    ]);
...
function publicError(err) {
  return {
    code: err.code || 'github_status_failed',
    message: err.message || 'GitHub接続確認に失敗しました。',
    github: err.github
  };
}
